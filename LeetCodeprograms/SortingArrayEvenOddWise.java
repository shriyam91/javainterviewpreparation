...
public class SortingArrayEvenOddWise {
    ...
    public static int[] sortEvenOdd(int[] array) {
        ...
        int[] evens = Arrays.stream(array).filter(n -> n % 2 == 0).sorted().toArray();
        int[] odds = Arrays.stream(array).filter(n -> n % 2 != 0).sorted().toArray();
